...
#----------------------------------------------------------------------------------------------------------
def band(t1,t2,a,Nk,mass,dim,totomega,graphene=True):
    print t1,t2,a,Nk
    if graphene:
	...
	n=0.0
		...
		omega=10.0/totomega*(Nomega-totomega/2)
    		...
    		for nkx in range (1,Nk):
    			for nky in range(1,Nk):
    				dosplus= dosplus+0.02/((omega-gapplus[nkx][nky])**2+(0.02)**2)
   				dosflat= dosflat+0.02/((omega-gapflat[nkx][nky])**2+(0.02)**2)
    				dosminus= dosminus+0.02/((omega-gapminus[nkx][nky])**2+(0.02)**2)
        	dostot=dosflat+dosminus
                n=n+(dosplus/(Nk*Nk))*(10.0/totomega)
		n1=n1+(dostot/(Nk*Nk))*(10.0/totomega)
    		f.write("%6.2f %6.2f %6.2f %6.2f\n" %(omega,dosplus/(Nk*Nk),dosflat/(Nk*Nk),dosminus/(Nk*Nk)))
	print n, n1
